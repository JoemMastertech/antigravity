...
const CONSTANTS = {
  ...
  MESSAGES: {
    SPECIAL: "Puedes elegir: 2 Jarras de jugo ├│ 5 Refrescos ├│ 1 Jarra de jugo y 2 Refrescos",
    ONLY_SODAS: "Puedes elegir hasta 5 refrescos",
    DEFAULT: "Puedes elegir hasta 5 acompa├▒amientos",
  ...
  PRODUCT_OPTIONS: {
    ...
    VODKA: ['Jugo de Pi├▒a', 'Jugo de Uva', 'Jugo de Naranja', 'Jugo de Ar├índano', 'Jugo de Mango', 'Jugo de Durazno', 'Mineral', 'Quina'],
    GINEBRA: ['Jugo de Pi├▒a', 'Jugo de Uva', 'Jugo de Naranja', 'Jugo de Ar├índano', 'Jugo de Mango', 'Jugo de Durazno', 'Mineral', 'Quina'],
...
class OrderSystem {
  ...
  handleDelegatedEvent(event) {
    ...
    if (target.id === 'keep-garnish-btn') {
      event.preventDefault();
      this._addMeatToOrder('Guarnici├│n est├índar');
      return;
    }
    ...
    // Handle price buttons: solo permitir si el modo orden est├í activo
    if (target && target.classList && target.classList.contains('price-button')) {
      ...
      // Bloquear selecci├│n si el modo orden no est├í activo y mostrar aviso
      if (!this.isOrderMode) {
        event.preventDefault();
        this._showValidationModal('Para agregar productos, primero presiona ÔÇ£Crear ordenÔÇØ en el men├║.');
        return;
      }
  ...
  handleProductSelection(productName, priceText, row, event) {
    if (!this._validateSelection(event)) {
      this._showValidationModal('Activa el modo de orden desde ÔÇ£Crear ordenÔÇØ para seleccionar productos.');
      return;
    }
  ...
  _handleLiquorProduct(productName, price) {
    ...
    if (isBottle && isSpecialCategory) {
      if (this.currentCategory === CONSTANTS.CATEGORIES.DIGESTIVOS) {
        ...
        if (CONSTANTS.SPECIAL_PRODUCTS.NO_MODAL.some(p => normalizedName.includes(p))) {
          return this.addProductToOrder({ name: `Botella ${productName}`, price, category: 'licor', customizations: ['Sin acompa├▒amientos'] });
        }
      } else {
        return this.addProductToOrder({ name: `Botella ${productName}`, price, category: 'licor', customizations: ['Sin acompa├▒amientos'] });
      }
    ...
    if (showModal) {
      ...
      this.addProductToOrder({ name: productName, price, category: 'licor', customizations: ['Revisar presentaci├│n'] });
    }
  }
  ...
  _continueShowDrinkOptionsModal() {
    ...
    // Re-aplicar traducci├│n para contenido din├ímico del modal
  ...
  _updateModalTitle() {
    ...
    const baseTitle = '┬┐Con qu├® desea acompa├▒ar su bebida?';
    const styleSpan = '<span class="modal-subtitle">';
    
    if (this.bottleCategory === 'VODKA' || this.bottleCategory === 'GINEBRA') {
      modalTitle.innerHTML = `${baseTitle}${styleSpan}Puedes elegir 2 Jarras de jugo ├│ 5 Refrescos ├│ 1 Jarra de jugo y 2 Refrescos</span>`;
    ...
    // Re-aplicar traducci├│n al actualizar el t├¡tulo
    this._retranslateIfNeeded(document.getElementById('drink-options-modal'));
  }

  // Re-traduce contenido nuevo del modal si el idioma actual no es espa├▒ol
  async _retranslateIfNeeded(scopeElement) {
    try {
      ...
      if (currentLang && currentLang !== 'es') {
        try {
          ...
          if (DOMTranslator && typeof DOMTranslator.translateElement === 'function') {
            ...
            // Fallback: traducir toda la p├ígina solo si no hay capacidad de traducci├│n por elemento
      ...
      Logger && Logger.warn ? Logger.warn('Fallo al re-aplicar traducci├│n en OrderSystem', error) : null;
    }
  }
  ...
  _createJagerMessage(container) {
    const messageElement = this._createElement('p', 'drink-options-message', 'Puedes elegir 5 Refrescos ├│ 2 Boost');
  ...
  _createNoneOption(option) {
    ...
    // Estabilizar traducci├│n y preservar texto original
  ...
  _createDrinkOption(option) {
    ...
    const optionName = this._createElement('span', 'drink-option-name');
    optionName.textContent = option;
    // Estabilizar traducci├│n y preservar texto original para la l├│gica
  ...
  updateTotalDrinkCount() {
    ...
    document.querySelectorAll('.drink-option-container .counter-btn[textContent="+"]').forEach(btn => {
      ...
      // Usar el texto original (no traducido) para la l├│gica de negocio
  ...
  _isOnlySodaCategory() {
    // Detectar subcategor├¡as que solo tienen refrescos
    // Esto se puede expandir seg├║n las categor├¡as espec├¡ficas del negocio
  ...
  getDrinkOptionsForProduct(productName) {
    ...
    if (!productName || typeof productName !== 'string') {
      ...
      return { drinkOptions: ['Ninguno'], message: 'Error: Producto no v├ílido' };
    }
    ...
    if (productType === 'ESPUMOSOS') {
      return { drinkOptions: ['Ninguno'], message: CONSTANTS.MESSAGES.NO_REFRESCOS || 'Sin acompa├▒amientos' };
    }
    ...
    // Determinar el mensaje apropiado seg├║n el tipo de producto
    let message;
    if (['VODKA', 'GINEBRA'].includes(productType)) {
      message = CONSTANTS.MESSAGES.SPECIAL || 'Opciones especiales';
    } else {
      // Verificar si es una categor├¡a solo de refrescos
      ...
      message = isOnlySodas ? 
        ...
        (CONSTANTS.MESSAGES.DEFAULT || 'Seleccione acompa├▒amiento');
    }
  ...
  _getSpecialProductOptions(normalizedName) {
    ...
    const specialProducts = {
      'BACARDI MANGO': ['Sprite', 'Mineral', 'Quina', 'Jugo de Mango', 'Jugo de Ar├índano'],
      'BACARDI RASPBERRY': ['Sprite', 'Mineral', 'Quina', 'Jugo de Mango', 'Jugo de Ar├índano'],
      'MALIBU': ['Sprite', 'Mineral', 'Jugo de Pi├▒a']
    };
  ...
  _getDigestivoOptions(normalizedName, productName) {
    ...
    if (!normalizedName || !productName || !this.currentProduct) {
      ...
      return { drinkOptions: ['Ninguno'], message: 'Sin acompa├▒amientos' };
    }
    ...
    if (this.currentProduct && this.currentProduct.priceType === CONSTANTS.PRICE_TYPES.BOTTLE) {
      ...
      for (const [key, options] of Object.entries(digestivoOptions)) {
        if (normalizedName.includes(key)) {
          return { 
            drinkOptions: Array.isArray(options) ? options : ['Ninguno'], 
            message: "Seleccione acompa├▒amiento:" 
          };
        }
      }
      return { 
        drinkOptions: ['Ninguno'], 
        message: CONSTANTS.MESSAGES.NO_REFRESCOS || 'Sin acompa├▒amientos' 
      };
    }
    
    if (this.currentProduct && this.currentProduct.priceType === CONSTANTS.PRICE_TYPES.CUP && productName.includes("BAILEYS")) {
      return { drinkOptions: ['Rocas'], message: "Acompa├▒amientos para copa" };
    }
  ...
  confirmDrinkOptions() {
    if (!this.currentProduct) {
      ...
      this._showValidationModal('Error: No se pudo confirmar la selecci├│n. Por favor intente nuevamente.');
      return;
    }
    
    if (!this._hasValidDrinkSelection()) {
      this._showValidationModal('Por favor seleccione al menos un acompa├▒amiento');
      return;
    }

    const { prefix, name, customization } = this._buildProductInfo();
    
    // Requerir modo orden activo antes de confirmar acompa├▒amientos
    if (!this.isOrderMode) {
      this._showValidationModal('Para confirmar acompa├▒amientos, primero activa ÔÇ£Crear ordenÔÇØ.');
      return;
    }
  ...
  _buildProductInfo() {
    ...
    if (priceType === 'precioBotella') {
      if (isJagerBottle && this.selectedDrinks.includes('2 Boost')) {
        ...
        customization = this.selectedDrinks.includes('Ninguno') ? 'Sin acompa├▒amientos' : `Con: ${this.selectedDrinks.join(', ')}`;
      }
  ...
  showLiterOptionsModal() {
    ...
    setTimeout(() => {
      const modalTitle = document.querySelector('#drink-options-modal h3');
      if (modalTitle) {
        modalTitle.innerHTML = '┬┐Con qu├® desea acompa├▒ar su bebida?<span class="modal-subtitle">Cada litro se sirve con 6 oz del destilado que elija.</span>';
        // Re-aplicar traducci├│n tras actualizar el t├¡tulo
        this._retranslateIfNeeded(document.getElementById('drink-options-modal'));
      }
    }, 10);
  ...
  showCupOptionsModal() {
    ...
    setTimeout(() => {
      const modalTitle = document.querySelector('#drink-options-modal h3');
      if (modalTitle) {
        modalTitle.innerHTML = '┬┐Con qu├® desea acompa├▒ar su bebida?<span class="modal-subtitle">Cada copa se sirve con 1 ┬¢ oz del destilado que elija.</span>';
        // Re-aplicar traducci├│n tras actualizar el t├¡tulo
        this._retranslateIfNeeded(document.getElementById('drink-options-modal'));
      }
    }, 10);
  ...
  _getOptionsForProduct(category, type) {
    ...
    const specialProducts = {
      'BACARDI MANGO': ['Sprite', 'Mineral', type === 'liter' ? 'Tonic' : 'Tonic', 'Jugo de Mango', 'Jugo de Ar├índano'],
      'BACARDI RASPBERRY': ['Sprite', 'Mineral', 'Tonic', 'Jugo de Mango', 'Jugo de Ar├índano'],
      'MALIBU': ['Sprite', 'Mineral', 'Jugo de Pi├▒a', type === 'liter' ? 'Mineral-Pi├▒a' : 'Mineral-Pi├▒a']
    };
    
    for (const [key, options] of Object.entries(specialProducts)) {
      if (productName.includes(key)) {
        return type === 'liter' ? { options, message: 'Elija una opci├│n para acompa├▒ar su litro:' } : options;
      }
    }
    ...
    const optionsMap = {
      ...
      'VODKA': ['Jugo de Pi├▒a', 'Jugo de Naranja', 'Jugo de Ar├índano', 'Jugo de Mango', 'Jugo de Uva', 'Jugo de Durazno', 'Mineral', 'Tonic'],
      'GINEBRA': ['Jugo de Pi├▒a', 'Jugo de Naranja', 'Jugo de Ar├índano', 'Jugo de Mango', 'Jugo de Uva', 'Jugo de Durazno', 'Mineral', 'Tonic'],
  ...
  _getDigestivoOptionsForCup(productName) {
    const digestivoMap = {
      'LICOR 43': ['Co├▒aquera Chaser Mineral', 'Rocas'],
      'J├äGERMEISTER': ['Derecho'],
      'BAILEYS': ['Rocas'],
      'CADENAS DULCE': ['Co├▒aquera Chaser Mineral', 'Rocas'],
      'ZAMBUCA NEGRO': [
        'Co├▒aquera Chaser Mineral-Moscas',
        'Co├▒aquera Chaser Mineral',
        'Rocas'  
      ]
    };
  ...
  _setupOptionsModal(type) {
    ...
    this._showModal('drink-options-modal');
    // Retraducci├│n para contenido renderizado din├ímicamente
    setTimeout(() => this._retranslateIfNeeded(document.getElementById('drink-options-modal')), 0);
  }
  ...
  _getOptionsForModalType(type) {
    ...
    if (productName.includes('BACARDI MANGO') || productName.includes('BACARDI RASPBERRY')) {
      return ['Sprite', 'Mineral', 'Tonic', 'Jugo de Mango', 'Jugo de Ar├índano'];
    }
    if (productName.includes('MALIBU')) {
      return ['Sprite', 'Mineral', 'Jugo de Pi├▒a', 'Mineral-Pi├▒a'];
    }
  ...
  _addMeatToOrder(garnishType) {
    ...
    this.addProductToOrder({
      ...
      price: this.currentProduct.price,
      category: 'comida',
      customizations: [`T├®rmino: ${this._getTermText(this.selectedCookingTerm)}`, garnishType]
    });
    this._hideModal('meat-customization-modal');
  }

  _getTermText(term) {
    const termMap = { 'medio': 'T├®rmino ┬¢', 'tres-cuartos': 'T├®rmino ┬¥', 'bien-cocido': 'Bien Cocido' };
  ...
  _confirmGarnishCustomization() {
    ...
    const garnishType = garnishModifications ? `Guarnici├│n: ${garnishModifications}` : 'Guarnici├│n est├índar';
  ...
  _validateCookingTerm() {
    ...
    if (!isValid) {
      OrderSystemValidations.showValidationModal(
        'Por favor seleccione un t├®rmino de cocci├│n primero',
  ...
  addProductToOrder(orderItem) {
    ...
    if (!this.isOrderMode) {
      this._showValidationModal('Para agregar productos, primero activa ÔÇ£Crear ordenÔÇØ.');
      return;
    }
  ...
  updateOrderDisplay() {
    ...
    if (!orderItemsContainer) {
      ...
      const sidebar = document.getElementById(CONSTANTS.SELECTORS.SIDEBAR);
      console.log('­ƒöì DEBUG - updateOrderDisplay: order-items not found');
      ...
      if (this.isOrderMode) {
        if (sidebar) {
          ...
          setTimeout(() => {
            ...
            console.log('­ƒöì DEBUG - updateOrderDisplay retry after 150ms:');
  ...
  completeOrder() {
    ...
    if (currentOrderItems.length === 0) {
      OrderSystemValidations.showValidationModal(
        'La orden est├í vac├¡a. Por favor agregue productos.',
        this._createSimpleModal.bind(this)
      );
      return;
    }

    this._showSuccessModal('┬íOrden completada con ├®xito!', () => {
      const order = {
        id: Date.now(), 
  ...
  getLiquorType(productName) {
    ...
    const BRAND_MAPPING = {
      'BACARD├ì': 'RON', 'HAVANA': 'RON', 'MATUSALEM': 'RON', 
      'APPLETON ESTATE': 'RON', 'CAPIT├üN MORGAN': 'RON', 'ZACAPA 23': 'RON',
      ...
      '400 CONEJOS': 'MEZCAL', 'AMAR├üS': 'MEZCAL', 'MONTELOBOS': 'MEZCAL', 
      'UNION': 'MEZCAL', 'TRIPAS DE MAGUEY': 'MEZCAL',
      
      'R├ëMY MARTIN': 'COGNAC', 'HENNESSY': 'COGNAC', 'MARTELL': 'COGNAC', 
      'COURVOISIER': 'COGNAC',
      
      'HIPNOTIQ': 'DIGESTIVOS', 
      'LICOR 43': 'DIGESTIVOS', 
      'J├äGERMEISTER': 'DIGESTIVOS', 
      'BAILEYS': 'DIGESTIVOS', 
  ...
  deleteOrder(orderId) {
    this._showConfirmationModal(
      '┬┐Est├í seguro de que desea eliminar esta orden?',
      'Esta acci├│n mover├í la orden al historial.',
      () => {
        ...
        if (ordersScreen && !ordersScreen.classList.contains('screen-hidden')) {
          if (this.isShowingHistory) {
            ...
            // Limpiar y repoblar solo la lista de ├│rdenes activas
            const ordersList = document.getElementById('orders-list');
            if (ordersList) {
              ordersList.innerHTML = '';
              this._populateOrdersList('orders-list', 'orders', 'No hay ├│rdenes guardadas.', true);
            }
          }
        }
      }
  ...
  showOrdersScreen() {
    ...
    this.previousTitle = elements.pageTitleElement ? elements.pageTitleElement.textContent : 'Cocteler├¡a';
    this.isShowingHistory = false;
    
    // Remover cualquier pantalla de ├│rdenes existente para evitar duplicados
    ...
    // Crear y agregar una nueva pantalla de ├│rdenes
  ...
  _createOrdersHeader() {
    ...
    const buttons = [
      ...
      { class: 'nav-button history-btn', text: 'Historial ├ôrdenes', handler: (btn) => this.toggleOrderHistoryView(btn) }
    ];
    
    const backButton = this._createButton(buttons[0]);
    const title = this._createElement('h2', 'orders-screen-title');
    title.textContent = '├ôrdenes Guardadas';
  ...
  _showHistoryView(button, { ordersList, orderHistoryContainer, ordersScreenTitle }) {
    ...
    button.textContent = 'Ver ├ôrdenes Activas';
    if (ordersScreenTitle) ordersScreenTitle.textContent = 'Historial de ├ôrdenes';
    if (ordersList) {
  ...
  _showActiveOrdersView(button, { ordersList, orderHistoryContainer, ordersScreenTitle }) {
    button.textContent = 'Historial ├ôrdenes';
    if (ordersScreenTitle) ordersScreenTitle.textContent = '├ôrdenes Guardadas';
    ...
    this.populateOrdersScreen();
      ...
      // Limpiar el bot├│n fijo al salir de la vista de historial
  ...
  populateOrderHistoryScreen(container) {
    ...
    // Agregar el contenedor de lista de ├│rdenes
    const ordersListContent = this._createElement('div', 'history-list-content');
    container.appendChild(ordersListContent);
    
    // Poblar el contenedor con las ├│rdenes del historial
    this._populateOrdersList(ordersListContent, 'orderHistory', 'No hay ├│rdenes en el historial.', false);
    
    // Crear un contenedor fijo en la parte inferior para el bot├│n de limpiar
  ...
  _createFixedBottomClearButton(parentContainer) {
    ...
    // Limpiar cualquier bot├│n fijo existente antes de crear uno nuevo
    ...
    // Crear el bot├│n de limpiar historial
    ...
    button.addEventListener('click', () => {
      // Encontrar el contenedor de lista de ├│rdenes para limpiar
    ...
    // Definir funci├│n para limpiar este bot├│n espec├¡fico
    this._cleanupFixedButton = function() {
      console.log("Cleaning up fixed bottom button");
      if (fixedContainer && fixedContainer.parentNode) {
        fixedContainer.parentNode.removeChild(fixedContainer);
      }
      // Reiniciar la funci├│n para evitar referencias colgantes
  ...
  // Modificar hideOrdersScreen para limpiar el bot├│n fijo
  async hideOrdersScreen() {
    console.log("desmontado"); // Log para validar el desmontaje
    
    // Limpiar el bot├│n fijo al salir de la pantalla de ├│rdenes
    console.log("Hiding orders screen - cleaning up button");
    this._cleanupFixedButton();
    
    try {
      // Ocultar la pantalla de ├│rdenes
      const ordersScreen = document.querySelector('.orders-screen');
      if (ordersScreen) {
      ...
      // Asegurarse de que la pantalla principal est├® visible
      ...
      // Restablecer el estado de visualizaci├│n de historial
      this.isShowingHistory = false;
      
      // Cargar el contenido de la categor├¡a anterior
      if (this.previousCategory) {
        await AppInit.loadContent(this.previousCategory);
      } else {
        await AppInit.loadContent('cocteles'); // Valor predeterminado si no hay categor├¡a anterior
      }
      
      // Mostrar el bot├│n de hamburguesa nuevamente
      ...
      console.error('Error al ocultar la pantalla de ├│rdenes:', error);
  ...
  _populateOrdersList(containerOrId, storageKey, emptyMessage, includeDeleteButton) {
    ...
    // Limpiar el contenedor antes de agregar ├│rdenes para evitar duplicaci├│n
    container.innerHTML = '';
  ...
  _createOrderElement(order, index, includeDeleteButton) {
    ...
    if (includeDeleteButton) {
      ...
      // Determinar si estamos en historial o en ├│rdenes activas
      if (this.isShowingHistory) {
  ...
  deleteFromHistory(orderId) {
    this._showConfirmationModal(
      '┬┐Est├í seguro de que desea eliminar esta orden del historial?',
      'Esta acci├│n eliminar├í permanentemente la orden del historial.',
      () => {
        // Solo eliminar del historial, NO afectar ├│rdenes activas
        const orderHistory = JSON.parse(localStorage.getItem('orderHistory') || '[]');
  ...
  promptClearHistory(historyContainer) {
    this._showConfirmationModal(
      '┬┐Est├í seguro de que desea limpiar el historial?',
      'Esta acci├│n no se puede deshacer.',
      () => {
        // Solo limpiar el historial, NO las ├│rdenes activas
        ...
        // NO afectar las ├│rdenes activas - mantener localStorage 'orders' intacto
        // Las ├│rdenes activas deben permanecer en su interfaz
      }
    );
  }
  ...
  async hideOrdersScreen() {
    ...
    Logger.debug('­ƒöä Ocultando pantalla de ├│rdenes - Estado DOM antes:', {
    ...
    // Mostrar bot├│n hamburguesa y ocultar pantalla de ├│rdenes
    elements.hamburgerBtn.classList.add('hamburger-visible');
    elements.hamburgerBtn.classList.remove('hamburger-hidden');
    
    // Limpiar y remover event listeners de la pantalla de ├│rdenes
    if (elements.ordersScreen) {
      ...
      // Eliminar la pantalla de ├│rdenes del DOM para una limpieza completa
    ...
    if (this.previousCategory && window.AppInit) {
      Logger.debug('­ƒô× Llamando a AppInit.loadContent con categor├¡a:', this.previousCategory);
      try {
        await window.AppInit.loadContent(this.previousCategory);
        
        // Log DOM state after loadContent
        setTimeout(() => {
          Logger.debug('­ƒôè Estado DOM despu├®s de loadContent:', {
            mainScreen: !!document.getElementById('main-screen'),
            contentContainer: !!document.getElementById('content-container'),
            ordersBox: !!document.getElementById('orders-box'),
        ...
        Logger.error('ÔØî Error al cargar contenido anterior:', error);
...
function initializeOrderSystem() {
  ...
  Logger.debug('­ƒöº Inicializando OrderSystem - Estado DOM:', {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      Logger.debug('Ô£à OrderSystem inicializado - Estado DOM despu├®s:', {
